...
def plot_confusion_matrix(file_path: str = "vz.root", out_path: str = "vz_confusion_matrix.pdf"):
    ...
    # interpolation="none" embeds the 4x4 image natively instead of upsampling it.
    # The np.nan-blanked empty cells make this a masked array, and LogNorm's
    # in-place subtract on a large *upsampled* masked array is pathologically slow
    # on some numpy builds (savefig hangs for minutes). With no resampling the norm
    # runs on the 16 cells, not millions of pixels.
    ...
    im1 = axs[1].imshow(frac, cmap="magma", vmin=0.0, vmax=1.0, aspect="auto", interpolation="none")
