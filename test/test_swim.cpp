...
struct Uniform : Field {
    double bz;
    explicit Uniform(double b) : bz(b) {}
    std::array<double, 3> b_kgauss(double, double, double) const override {
        return {0.0, 0.0, bz};
    }
};
...
int main() {
    ...
    {
        Uniform field(0.0);
        SwimResult r = swim_back_to_beamline(field, {10.0, 0.0, 300.0}, {2.0, 0.0, 5.0}, 1.0);
        std::fprintf(stderr, "zero-field: status=%d vz=%.6f doca=%.3e path=%.3f\n",
                     static_cast<int>(r.status), r.vz, r.doca_rho, r.path);
        check(r.status == SwimStatus::Converged, "zero-field converged");
    ...
    // Uniform Bz -> helix: should still reach a closest approach within max path.
    {
        ...
        SwimResult r = swim_back_to_beamline(field, {10.0, 0.0, 300.0}, {0.3, 0.0, 1.0}, 1.0);
        std::fprintf(stderr, "helix: status=%d vz=%.6f doca=%.3e path=%.3f\n",
                     static_cast<int>(r.status), r.vz, r.doca_rho, r.path);
        check(r.status == SwimStatus::Converged, "helix converged");
