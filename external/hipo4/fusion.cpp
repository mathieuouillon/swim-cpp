...
namespace hipo {
  ...
  void inputSource::open(const char *filename, int tag) {
    reader.setTags(tag);
    reader.open(filename);
    reader.readDictionary(factory);
  }
...
double  inputSource::getDouble( const char *bank, const char *entry, int row){
  ...
  int fusion::open(const char *filename, int tag) {
    int number = sources.size() + 1;
    sources.insert(std::make_pair(number, new inputSource(filename, tag)));
    return number;
  }
